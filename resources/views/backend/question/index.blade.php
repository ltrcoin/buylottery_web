...
@if (Session::has('msg_question'))
   	<script type="text/javascript">
   	$(function() {
   		jAlert('{{Session::get("msg_question")}}', 'Thông báo');
   	});
   	</script>
@endif
		...
		<div class="col-sm-12">
			<div class="box box-primary">
				<div class="box-header with-border">
				  	<div class="row">
				  		<div class="col-sm-2">
							<a class="btn btn-primary" href="{{route('backend.question.vAdd')}}">Thêm mới câu hỏi</a>
						</div>
						<div class="col-sm-3">
					...
					<div id="example1_wrapper" class="dataTables_wrapper form-inline dt-bootstrap">
						<div class="row">
							<div class="col-sm-1">
								...
								<input type="hidden" id="delUrl" value="{{url('admin/question/delete')}}">
							</div>
							<div class="col-sm-11">
						...
						<div class="row">
							<div class="col-sm-12">
								<table id="example1" class="table table-bordered table-striped dataTable" role="grid" aria-describedby="example1_info">
				        			...
				        			<tbody>
				        				@forelse ($listQuestion as $key => $item)  
				              			<tr class="{{$key%2 == 0 ? 'even' : 'odd'}}">
				              				<td>
				              					<input type="checkbox" class="checkItem" value="{{$item->id}}">
			              					</td>
							          		<td>{{isset($listType[$item->type]) ? $listType[$item->type] : ''}}</td>
							          		<td>{{isset($listObj[$item->object]) ? $listObj[$item->object] : ''}}</td>
							          		<td>{{$item->name}}</td>
								          	<td class="text-center">
								          		<a href="{{route('backend.question.vEdit',['id'=>$item->id])}}" class="editItem" id="" title="Sửa"><i class="fa fa-fw fa-edit"></i></a>
		      			...
		      			<div class="row">
	      					...
	      					<div class="col-sm-7">
	      						<div class="dataTables_paginate paging_simple_numbers" id="example1_paginate">
	      							{{ $listQuestion->appends($filter)->links() }}
	      						</div>
	      					</div>
	      				</div>
      				</div>    
	...
	<div class="modal-dialog" role="document">
    	<div class="modal-content">
  			...
  			<div class="form-group">
      			<form id="searchForm" action="{{route('backend.question.index')}}">
	      			<div class="modal-body">
	        			<div class="form-group">
		        			<label for="message-text" class="control-label">Chọn loại:</label>
		        			<select class="form-control" id="sType" name="type" style="width: 50%;">
		          				...
		          				@if(isset($listType) && count($listType) > 0)
					              	...
					              	<option value="{{$id}}" {{(isset($filter['type']) && $filter['type'] == $id) ? 'selected' : ''}}>{{$item}}</option>
					              	@endforeach
					            @endif
		        			</select>
		      			</div>
		      			<div class="form-group">
		        			<label for="recipient-name" class="control-label">Tên câu hỏi:</label>
