...
<section class="content">
	<div class="row">
		<div class="col-md-12">
			<form action="{{route('backend.question.pEdit', ['id' => $model->id])}}" method="POST">
				{{csrf_field()}}
				      	...
				      	<div class="row">
					        <div class="col-md-6">
					        	<div class="form-group {{$errors->has('type') ? 'has-error' : ''}}">
						            ...
						            <select class="form-control select2" name="type" style="width: 100%;">
						              	...
						              	@if(isset($listType) && count($listType) > 0)
							              	...
							              	<option value="{{$id}}" {{$model->type == $id ? 'selected' : ''}}>{{$item}}</option>
							              	@endforeach
							            @endif
						            </select>
					          	...
					          	<div class="form-group {{$errors->has('object') ? 'has-error' : ''}}">
						            ...
						            <select class="form-control select2" name="object" style="width: 100%;">
						              	...
						              	@if(isset($listObj) && count($listObj) > 0)
							              	...
							              	<option value="{{$id}}" {{$model->object == $id ? 'selected' : ''}}>{{$item}}</option>
							              	@endforeach
							            @endif
						            </select>
					        	...
					        	<div class="form-group {{$errors->has('name') ? 'has-error' : ''}}">
					              	...
					              	<textarea class="form-control" name="name" id="name" placeholder="Nhập nội dung câu hỏi" rows="5">{{$model->name}}</textarea>
