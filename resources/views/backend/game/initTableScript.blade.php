...
{{--<script>--}}
    ...
    columnDefs: [
      ...
      },
      {
        ...
        render: function (data, type, row, meta) {
          if(!data) {
            data = '<a href="javascript:void(0)"' +
            '   class="draw_now btn btn-sm btn-primary" data-id="' + row[0] + '" title="{{__('label.game.draw_now')}}">{{ __('label.game.draw_now') }}</a>'
          } else {
            data = '{{ __('label.game.from_api') }}';
          }
  ...
  $(document).on('click', '.draw_now', function() {
    $.ajax({
      url: '/game/update-result-game/' + $(this).data('id'),
      type: 'GET',
      success: function (response) {
        jAlert(response.msg, '{{ __('messages.alert') }}')
      }
    })
  });