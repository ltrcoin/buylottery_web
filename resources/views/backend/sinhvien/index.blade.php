...
@section('title', 'HS Sinh viên')
@section('content')
<link rel="stylesheet" href="{{asset('backend/plugins/datatables/dataTables.bootstrap.css')}}">
@if (Session::has('msg_sinhvien'))
   	<script type="text/javascript">
   	$(function() {
   		jAlert('{{Session::get("msg_sinhvien")}}', 'Thông báo');
   	});
   	</script>
@endif
<section class="content-header">
  <h1>
    Danh sách hs sinh viên
    <small>Control panel</small>
  </h1>
  <ol class="breadcrumb">
    <li><a href="{{route('backend.site.index')}}"><i class="fa fa-dashboard"></i> Home</a></li>
    <li class="active">Danh sách hs sinh viên</li>
  </ol>
</section>
		...
		<div class="col-sm-12">
			<div class="box box-primary">
				<div class="box-header with-border">
				  	<div class="row">
						<div class="col-sm-2">
							<a class="btn btn-primary" href="{{route('backend.sinhvien.vAdd')}}">Thêm mới hs sinh viên</a>
						</div>
						<div class="col-sm-3">
				...
				<div class="box-body">
					<div id="example1_wrapper" class="dataTables_wrapper form-inline dt-bootstrap">
						<div class="row">
							<div class="col-sm-1">
								<button type="button" class="btn btn-danger deleteAll">Xóa</button>
								<input type="hidden" id="delUrl" value="{{url('admin/sinhvien/delete')}}">
							</div>
							<div class="col-sm-11">
								...
								<table id="example1" class="table table-bordered table-striped dataTable" role="grid" aria-describedby="example1_info">
							        <thead>
							        	<tr>
							        		...
							        		<th width="25%">Tên ngành</th>
							        		<th width="25%">Tên khoa</th>
							        		<th width="7%">Action</th>
						        		</tr>
				        			</thead>
				        			<tbody>
				        				@forelse ($listSinhvien as $key => $item)
				              			<tr class="{{$key%2 == 0 ? 'even' : 'odd'}}">
				              				<td>
				              					<input type="checkbox" class="checkItem" value="{{$item->idmsv}}">
			              					</td>
							          		<td>{{$item->idmsv}}</td>
							          		<td>{{$item->hoten}}</td>
								          	<td>{{isset($listGioitinh[$item->gioitinh]) ? $listGioitinh[$item->gioitinh] : ''}}</td>
								          	<td>{{$item->ngaysinh}}</td>
								          	<td>{{$item->nganh}}</td>
								          	<td>{{isset($listKhoa[$item->khoa]) ? $listKhoa[$item->khoa] : ''}}</td>
								          	<td class="text-center">
								          		<a href="{{route('backend.sinhvien.vEdit',['id'=>$item->idmsv])}}" class="editItem" id="" title="Sửa"><i class="fa fa-fw fa-edit"></i></a>
								          		<a href="javascript: void(0);" class="deleteItem" id="{{$item->idmsv}}" title="Xóa"><i class="fa fa-fw fa-remove"></i></a>
								          	</td>
								        </tr>
								        @empty
								    	<tr class="even">
		      			...
		      			<div class="row">
	      					...
	      					<div class="col-sm-7">
	      						<div class="dataTables_paginate paging_simple_numbers" id="example1_paginate">
	      							{{ $listSinhvien->appends($filter)->links() }}
	      						</div>
	      					</div>
	      				</div>
      				</div>    
