<div class="box-header with-border">
        <h3 class="box-title">{{ __('label.organize.investment_funds') }}</h3>
    </div>
    ...
    <div class="box-body">
        ...
        <div class="form-group">
            <label>{{ __('label.organize.investment_phase') }}<span style="color: #f00">*</span></label>
            <select name="investment_phase" class="form-control select2" style="width: 100%;">
                @foreach($lstInvestmentPhase as $data)
                    <option value="{{$data->code}}" {{ (isset($organize->investment_phase) && strpos($organize->investment_phase, $data->code) !== false) ? 'selected' : '' }} >
                        {{$data->name}}
                    </option>
                @endforeach
            </select>
        ...
        <div class="form-group">
            <label>{{ __('label.organize.value_of_investment') }}<span style="color: #f00">*</span></label>
            <select name="value_of_investment" class="form-control select2" style="width: 100%;">
                @foreach($lstValue_of_investment as $data)
                    <option value="{{$data->code}}" {{ (isset($organize->value_of_investment) && strpos($organize->value_of_investment, $data->code) !== false) ? 'selected' : '' }} >
                        {{$data->name}}
                    </option>
                @endforeach
            </select>
        ...
        <div class="form-group">
            ...
            <input class="form-control" value="{{ isset($organize->investment_capital) ? $organize->investment_capital : '' }}" name="investment_capital" type="number">
        </div>
    </div>