...
<section class="content-header">
  ...
  <ol class="breadcrumb">
    ...
    <li class="active">Cập nhật giảng viên</li>
  </ol>
</section>
<section class="content">
	<div class="row">
		<div class="col-md-12">
			<form action="{{route('backend.giangvien.pEdit',['id'=>$model->id])}}" method="POST" enctype="multipart/form-data">
				{{csrf_field()}}
				      	...
				      	<div class="row">
					        <div class="col-md-6">
					        	...
					        	<div class="form-group {{$errors->has('hoten') ? 'has-error' : ''}}">
					              	...
					              	<input type="text" class="form-control" name="hoten" id="hoten" placeholder="Enter tên giảng viên" value="{{$model->hoten}}">
					              	<span class="help-block">{{$errors->first("hoten")}}</span>
					            </div>
					            <div class="form-group">
					              	<label class="" for="alias">Khối ngành</label>
					              	<input type="text" class="form-control" name="khoinganh" id="khoinganh" placeholder="Enter khoinganh" value="{{$model->khoinganh}}">
					            </div>
					          	<div class="form-group {{$errors->has('trinhdo') ? 'has-error' : ''}}">
						            ...
						            <select class="form-control" id="trinhdo" name="trinhdo" style="width: 50%;">
						            	@if(isset($listTrinhdo))
							              	...
							              	<option value="{{$name}}" {{$model->trinhdo == $name ? 'selected' : ''}}>{{$name}}</option>
							              	@endforeach
							            @endif
						            </select>
