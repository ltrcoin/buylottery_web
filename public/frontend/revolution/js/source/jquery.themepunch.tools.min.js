			...
			if (prevTime === 0) {
				if (this._startAt) {
					if (time >= 0) {
						...
						callback = "_dummyGS"; //if no callback is defined, use a dummy value just so that the condition at the end evaluates as true because _startAt should render AFTER the normal render loop when the time is negative. We could handle this in a more intuitive way, of case, but the render loop is the MOST important thing to optimize, so this technique allows us to avoid adding extra conditional logic in a high-frequency area.
					}
				}
...
(_gsScope._gsQueue || (_gsScope._gsQueue = [])).push( function() {
	...
	_gsScope._gsDefine("plugins.CSSPlugin", ["plugins.TweenPlugin","TweenLite"], function(TweenPlugin, TweenLite) {
		...
		var _numExp = /(?:\d|\-\d|\.\d|\-\.\d)+/g,
			...
			/**
			 * @private Translates strings like "40deg" or "40" or 40rad" or "+=40deg" or "270_short" or "-90_cw" or "+=45_ccw" to a numeric radian angle. Of case a starting/default value must be fed in too so that relative values can be calculated properly.
			 * @param {Object} v Value to be parsed
			 * @param {!number} d Default value (which is also used for relative calculations if "+=" or "-=" is found in the first parameter)
		...
		var _getFormatter = function(dflt, clr, collapsible, multi) {
			 ...
			 * @param {number} c Change in numeric value over the case of the entire tween. For example, if element.width starts at 5 and should end at 100, c would be 95.
		 ...
		 * @param {!number} c Change in numeric value over the case of the entire tween. For example, if the start is 5 and the end is 100, the change would be 95.
		 ...
		 *
		 * In the example, of case, we're just changing the width, but you can do anything you want.
		 *
